...
def anndata_preprocess(adata_spa, adata_rna, highly_variable=2000, spatial_labels=False):
    ...
    if spatial_labels:
        ...
        common_cell_type=[i for i in np.unique(adata_rna.obs['cell_type']) if i in np.unique(adata_spa.obs['cell_type'])]
        novel_cell_type=[i for i in np.unique(adata_spa.obs['cell_type']) if i not in np.unique(adata_rna.obs['cell_type'])]
    ...
    sc.pp.highly_variable_genes(adata_cm, n_top_genes=highly_variable, batch_key='domain_id', inplace=False, subset=True)
...
def generate_dataloaders(adata_cm,adata_spa,adata_rna,batch_size=256):
    ...
    test_source_cm_dl = DataLoader(dataset=source_cm_ds, batch_size=batch_size, shuffle=True,
